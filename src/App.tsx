...
import {
  clearSession, fetchPlans, fetchProfile, fetchVideos, getStoredUser, hasRuntimeConfig,
  invokeVerifier, requestPasswordReset, signIn, signUp, submitSuggestion, type Plan, type SessionUser, type Video,
...
function Brand() {
  return <Link href="/" className="brand-mark focus-tv" data-testid="link-brand"><Clapperboard size={22} strokeWidth={1.8} /><span>CineVito</span></Link>;
}
...
function useVideos() {
  ...
  const [loading, setLoading] = useState(hasRuntimeConfig);
  const [error, setError] = useState('');
  useEffect(() => {
    if (!hasRuntimeConfig) return;
    fetchVideos().then((items) => { if (items.length) setVideos(items); }).catch((fetchError) => setError(fetchError instanceof Error ? fetchError.message : 'Não foi possível carregar o catálogo.')).finally(() => setLoading(false));
  }, []);
...
function SubscriptionPage() {
  ...
  useEffect(() => { if (hasRuntimeConfig) fetchPlans().then(setPlans).catch(() => setPlans([])).finally(() => setLoading(false)); }, []);
  ...
  return <div className="content-wrap page-main"><PageHeader eyebrow="Escolha o seu acesso" title="Assine o CineVito" description="Assista ao catálogo completo em seus dispositivos. O pagamento é processado com segurança pelo Mercado Pago." /><div className="two-col"><section className="panel panel-pad"><h2 className="panel-title">Planos disponíveis</h2>{loading ? <div className="plan-list">{[1, 2].map((n) => <div className="skeleton" style={{ height: 94 }} key={n} />)}</div> : plans.length ? <div className="plan-list">{plans.map((plan) => <div className={`plan-card ${selected === plan.id ? 'selected' : ''}`} key={plan.id} onClick={() => setSelected(plan.id)}><div><h3>{plan.nome}</h3><p>{plan.descricao || `${plan.limite_dispositivos || 1} dispositivo(s) · ${duration(plan)}`}</p></div><div style={{ display: 'grid', justifyItems: 'end', gap: 8 }}><strong className="plan-price">{typeof plan.preco === 'number' ? `R$ ${plan.preco.toFixed(2).replace('.', ',')}` : 'Consultar'}</strong><button className="primary-button focus-tv" onClick={() => setSelected(plan.id)} data-testid={`button-select-plan-${plan.id}`}>{selected === plan.id ? 'Selecionado' : 'Escolher'}</button></div></div>)}</div> : <div className="empty-state"><Settings size={24} /><h3>Planos em configuração</h3><p>Quando os planos estiverem publicados, eles aparecerão aqui para checkout.</p></div>}</section><aside className="panel panel-pad"><div className="eyebrow">Pagamento seguro</div><h2 className="panel-title" style={{ marginTop: 9 }}>Mercado Pago</h2><p className="muted" style={{ fontSize: '.82rem', lineHeight: 1.65 }}>O CineVito mantém o fluxo de checkout transparente com cartão e Pix. Nenhum dado de cartão fica salvo nesta interface.</p><div className="notice notice-orange" style={{ marginTop: 16 }}><Info size={16} color="#ff8228" /><span>O checkout só fica disponível quando o ambiente estiver conectado ao Supabase e ao Mercado Pago.</span></div></aside></div></div>;
}
...
function AdminPage() {
  ...
  const [profileLoading, setProfileLoading] = useState(Boolean(user && hasRuntimeConfig));
  const [url, setUrl] = useState('https://www.pobreflixtv.city/');
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
   const [result, setResult] = useState<{ links?: Array<{ url: string; status?: string; detalhe?: string; tipo?: string }>; total?: number } | null>(null);
  useEffect(() => {
    if (!user || !hasRuntimeConfig) return;
    setProfileLoading(true);
    fetchProfile().then((value) => setProfile(value as { is_admin?: boolean })).catch(() => setProfile(null)).finally(() => setProfileLoading(false));
  }, [user]);
  async function verify(event: FormEvent) {
    event.preventDefault();
    setError('');
    setResult(null);
    if (!url.startsWith('https://')) { setError('Informe uma URL HTTPS válida.'); return; }
    setBusy(true);
    try {
      const response = await invokeVerifier(url);
      if (response.error) setError(response.error);
      else setResult(response);
    } catch (verifyError) { setError(verifyError instanceof Error ? verifyError.message : 'Não foi possível executar o verificador.'); }
    finally { setBusy(false); }
  }
  if (!user) return <LockedPage title="Área administrativa" description="Entre com uma conta autorizada para acessar este painel." />;
  if (!hasRuntimeConfig) return <div className="content-wrap page-main"><PageHeader eyebrow="Administração" title="Área protegida" description="O painel administrativo só fica disponível quando a configuração do Supabase estiver presente." /><div className="notice notice-orange"><KeyRound size={17} color="#ff8228" /><span>Configure <strong>VITE_SUPABASE_URL</strong> e <strong>VITE_SUPABASE_ANON_KEY</strong> no ambiente. Nenhuma credencial foi criada neste artefato.</span></div></div>;
  if (profileLoading) return <div className="content-wrap page-main"><PageHeader eyebrow="Administração" title="Verificando acesso" description="Confirmando as permissões da sua conta." /><div className="skeleton" style={{ height: 120 }} data-testid="status-admin-loading" /></div>;
  if (!profile?.is_admin) return <LockedPage title="Acesso restrito" description="Esta área é exclusiva para administradores CineVito." />;
   return <div className="content-wrap page-main"><PageHeader eyebrow="Painel protegido" title="Administração" description="Gerencie o catálogo, clientes e ferramentas internas do CineVito." /><div className="admin-tabs" role="tablist">{['videos', 'coleções', 'clientes', 'planos', 'indicações', 'equipe', 'verificar'].map((item) => <button key={item} className={`admin-tab focus-tv ${tab === item ? 'active' : ''}`} onClick={() => setTab(item)} role="tab" aria-selected={tab === item} data-testid={`tab-admin-${item}`}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div>{tab === 'verificar' ? <section className="panel panel-pad"><div className="eyebrow">Ferramenta de integração</div><h2 className="panel-title" style={{ marginTop: 8 }}>Verificador de fontes de vídeo</h2><p className="muted" style={{ fontSize: '.82rem', lineHeight: 1.6, maxWidth: 720 }}>Analise a home ou uma página individual autorizada. O CineVito encontra arquivos diretos, players incorporados e páginas com <code>data-video-id</code>; a home do PobreFlixTV normalmente carrega os servidores depois, por JavaScript. Nada é salvo no banco.</p><form className="verifier-form" onSubmit={verify}><div className="field" style={{ marginBottom: 0 }}><label htmlFor="verify-url">URL da página para analisar</label><input id="verify-url" className="input focus-tv" type="url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://www.pobreflixtv.city/" data-testid="input-verifier-url" /></div><button className="primary-button focus-tv" type="submit" disabled={busy} data-testid="button-run-verifier"><RefreshCw size={16} />{busy ? 'Verificando...' : 'Verificar fontes'}</button></form>{error && <p className="form-error" style={{ marginTop: 15 }} data-testid="status-verifier-error">{error}</p>}{result && <div className="verifier-results" data-testid="status-verifier-results"><div className="notice notice-cyan"><Check size={17} color="#56e0ff" /><span><strong>{result.total ?? result.links?.length ?? 0} fonte(s) encontrada(s)</strong></span></div>{result.links?.map((link, index) => <div className="result-row" key={`${link.url}-${index}`}><span style={{ overflowWrap: 'anywhere' }}><strong style={{ display: 'block', marginBottom: 3 }}>{link.tipo === 'pagina' ? 'Página individual' : link.tipo === 'embed' ? 'Player incorporado' : 'Arquivo de vídeo'}</strong>{link.url}</span><span className={link.status === 'ativo' ? 'result-ok' : link.status === 'erro' ? 'result-error' : 'result-warn'}>{link.status || link.detalhe || 'analisado'}</span></div>)}</div>}</section> : <AdminPlaceholder tab={tab} />}</div>;
}

function AdminPlaceholder({ tab }: { tab: string }) {
  const descriptions: Record<string, string> = { videos: 'Adicione e organize vídeos do catálogo.', 'coleções': 'Monte coleções e defina sua ordem de exibição.', clientes: 'Consulte clientes, assinaturas e acessos concedidos.', planos: 'Configure planos pagos. Acesso gratuito individual é concedido na aba Clientes.', 'indicações': 'Crie e ative uma campanha por vez.', equipe: 'Gerencie os acessos administrativos da equipe.' };
  return <section className="panel panel-pad"><div className="empty-state"><Settings size={24} /><h3>{tab}</h3><p>{descriptions[tab]}</p><span className="muted" style={{ fontSize: '.72rem', marginTop: 8 }}>O fluxo original permanece protegido por Supabase RLS e está pronto para receber seus dados.</span></div></section>;
}
