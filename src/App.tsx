...
function getEmbedInfo(url?: string | null): { type: 'file' | 'embed' | 'none'; src: string } {
  ...
  // MixDrop: o link "/f/ID" é a página de assistir, que tem um script
  // que detecta se está dentro de um iframe e expulsa o vídeo pra fora.
  // O link "/e/ID" é a versão feita pra ser incorporada, sem esse bloqueio.
  // Convertemos preservando o domínio exato que o admin colou (mixdrop.co,
  // .ag, .to, .sx etc. mudam com o tempo por causa de bloqueios).
  const mixdropAssistir = trimmed.match(/^(https?:\/\/[^/]*mixdrop\.[a-z]+)\/f\/([a-zA-Z0-9]+)/i);
  if (mixdropAssistir) return { type: 'embed', src: `${mixdropAssistir[1]}/e/${mixdropAssistir[2]}` };
  if (/mixdrop\.[a-z]+\/e\//i.test(trimmed)) return { type: 'embed', src: trimmed };
