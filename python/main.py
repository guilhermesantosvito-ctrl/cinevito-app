...
DOMINIO_PERMITIDO = "zencdn.net"
URL_ALVO_PADRAO = "https://zencdn.net"
...
def validar_url_alvo(url_alvo: str) -> str:
    ...
    if host != DOMINIO_PERMITIDO and not host.endswith(f".{DOMINIO_PERMITIDO}"):
        raise ValueError("A análise fica limitada ao domínio zencdn.net.")
...
def verificar_videos(url_alvo: str = URL_ALVO_PADRAO) -> dict:
    ...
    try:
        response = httpx.get(url_alvo, timeout=10.0, follow_redirects=True)
        response.raise_for_status()
    except httpx.TimeoutException as erro:
        raise RuntimeError("Tempo limite excedido ao acessar a página alvo.") from erro
    except httpx.HTTPStatusError as erro:
        raise RuntimeError(
            f"A página alvo respondeu com o código HTTP {erro.response.status_code}."
        ) from erro
    except httpx.HTTPError as erro:
        raise RuntimeError(f"Erro ao acessar a página alvo: {erro}") from erro

    links = extrair_links_de_video(response.text, url_alvo)
    resultados = []
    for link in links:
        try:
            checagem = httpx.head(link, timeout=5.0, follow_redirects=True)
            if checagem.status_code == 200:
                resultados.append(
                    {"url": link, "status": "ativo", "status_code": 200,
                     "mensagem": "Link ativo e funcionando."}
                )
            else:
                resultados.append(
                    {"url": link, "status": "erro",
                     "status_code": checagem.status_code,
                     "mensagem": "Link respondeu, mas com erro."}
                )
        except httpx.TimeoutException:
            resultados.append(
                {"url": link, "status": "indisponivel", "status_code": None,
...
def buscar_e_validar(url_alvo: str | None = None) -> dict | None:
    ...
    try:
        response = httpx.get(alvo, timeout=10.0, follow_redirects=True)
    ...
    for link in links:
        ...
        try:
            ...
            if checagem.status_code == 200:
                ...
                resultados.append({"url": link, "status": "ativo", "status_code": 200})
            else:
                print(f"⚠️ Link respondeu, mas com erro. Código: {checagem.status_code}")
                resultados.append(
                    {"url": link, "status": "erro", "status_code": checagem.status_code}
                )
        except Exception:
            print("❌ Link quebrado ou fora do ar.")
            resultados.append({"url": link, "status": "indisponivel", "status_code": None})

    return {"source_url": alvo, "links_encontrados": len(links), "resultados": resultados}


def main() -> None:
    parser = argparse.ArgumentParser(description="Verifica links .mp4 e .m3u8.")
    parser.add_argument("--url", default=URL_ALVO_PADRAO, help="Página zencdn.net para analisar.")
    ...
    print(f"Encontramos {resultado['links_encontrados']} link(s) de vídeo.")
    for item in resultado["resultados"]:
        codigo = f" (HTTP {item['status_code']})" if item["status_code"] else ""
        print(f"{item['url']}: {item['mensagem']}{codigo}")
